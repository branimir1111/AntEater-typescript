...
type EditPMTicketProps = {
  allPMProjects: ProjectResponse[];
  ticket: TicketResponse;
};

const EditPMTicket = ({ allPMProjects, ticket }: EditPMTicketProps) => {
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-[425px] bg-background-first">
        <DialogHeader>
          <DialogTitle className="text-indigo-500">Edit Ticket</DialogTitle>
          <DialogDescription>
        ...
        <Form
          method="post"
          action={`/dashboard/manager-ticket/edit-pm-ticket/${_id}`}
        >
          <FormInput
            name="title"
            type="text"
