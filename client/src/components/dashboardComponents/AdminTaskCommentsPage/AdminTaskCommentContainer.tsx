import { type AdminTaskCommentsProps } from '@/utils';
import { Avatar, AvatarFallback, AvatarImage } from '@/components/ui/avatar';
import { CircleUserRound } from 'lucide-react';
import day from 'dayjs';
import { Separator } from '@/components/ui/separator';
import { DeleteCommentTask } from '@/components';
import { Badge } from '@/components/ui/badge';

type AdminTaskCommentContainerProps = {
  allTaskComments: AdminTaskCommentsProps[];
};

const AdminTaskCommentContainer = ({
  allTaskComments,
}: AdminTaskCommentContainerProps) => {
  return (
    <div className="w-full mt-4 grid break12:grid-cols-2 break17:grid-cols-3 gap-2">
      {allTaskComments.map((comment) => {
        const {
          _id,
          createdAt,
          text,
...
export default AdminTaskCommentContainer;
